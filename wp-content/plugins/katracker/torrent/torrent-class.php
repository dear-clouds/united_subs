...
// get torrent infos
$torrent = new TorrentFile( './test.torrent' );
...
// create torrent
$torrent = new TorrentFile( array( 'test.mp3', 'test.jpg' ), 'http://torrent.tracker/annonce' );
...
class TorrentFile {
	...
	* - TorrentFile(); // get an instance ( usefull to scrape and check errors )
	* - TorrentFile( string $torrent ); // analyse a torrent file
	* - TorrentFile( string $torrent, string $announce );
	* - TorrentFile( string $torrent, array $meta );
	* - TorrentFile( string $file_or_folder ); // create a torrent file
	* - TorrentFile( string $file_or_folder, string $announce_url, [int $piece_length] );
	* - TorrentFile( string $file_or_folder, array $meta, [int $piece_length] );
	* - TorrentFile( array $files_list );
	* - TorrentFile( array $files_list, string $announce_url, [int $piece_length] );
	* - TorrentFile( array $files_list, array $meta, [int $piece_length] );
	...
	*/
	...
	/** Convert the current TorrentFile instance in torrent format
	...
	*/
	static public function is_torrent ( $file ) {
		return self::file_get_contents( $file, 0, 11 ) === 'd8:announce' || self::file_get_contents( $file, 0, 14 ) === 'd10:created by';
	}
	...
	*/
	static public function file_get_contents ( $file, $offset = null, $length = null ) {
		return is_file( $file ) ? ! is_null( $offset ) ? $length ?
		       	@file_get_contents( $file, false, null, $offset, $length ) :
		       	@file_get_contents( $file, false, null, $offset ) :
		       	@file_get_contents( $file, false, null ) :
		       	null;
	}
