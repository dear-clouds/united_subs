 ...
 */
class MovDB_DeletableSources_List_Table extends WP_List_Table
{
	/**
	...
	*/
	function extra_tablenav ($which)
	{
		...
		echo '<label for="quality_threshold">' . __('Quality threshold', 'movdb') . '</label>';
		echo '<input type="range" min="1" max="10" step="1" value="' . $this->get_current_quality_threshold() . '" name="quality_threshold" id="quality_threshold" />';
		submit_button(__('Filter'), 'button', 'changeit', false);
		echo '</div>';
	}
	...
	**************************************************************************/
	...
	{
		return $this->format_rating_column($item['rating']);
	}
	...
	**************************************************************************/
	function prepare_items ()
	{
		...
		*/
		...
		$orderby = (!empty($_REQUEST['orderby'])) ? $_REQUEST['orderby'] : 'quality_rating'; // if no sort, set default here
		$order = (!empty($_REQUEST['order'])) ? $_REQUEST['order'] : 'asc'; // if no order, default to asc
		
		$query  = 'select src.id, src.movie, mov.title, mov.org_title, mov.short_title, mov.version, mov.series, mov.part, mov.year, mov.rating, src.type, src.audio_quality, src.video_quality, src.video_3d, src.audience_capable, ';
		$query .= 'trim(replace(replace(replace(replace(mov.title, "Der", ""), "Die", ""), "Das", ""), "The", "")) as title_sort, ';
		$query .= '((mov.rating * 1) + (src.audio_quality * 2) + (src.video_quality * 2)) / 5 as quality_rating ';
		$query .= 'from ' . $wpdb->prefix . 'movdb_sources src ';
		$query .= 'left outer join ' . $wpdb->prefix . 'movdb_movies mov on mov.id = src.movie ';
		$query .= 'left outer join ' . $wpdb->prefix . 'movdb_screenings scr on scr.movie = src.movie and scr.source = src.id ';
		$query .= 'where ';
		
		$query .= 'not scr.id is null and ((mov.rating * 1) + (src.audio_quality * 2) + (src.video_quality * 2)) / 5 <= ' . $this->get_current_quality_threshold() . ' ';
	...
	function get_current_quality_threshold ()
	{
		return isset($_GET['quality_threshold']) ? intval($_GET['quality_threshold']) : 5;
	}
}