 ...
 */
class MovDB_Screenings_List_Table extends WP_List_Table
{
	/**
	...
	***************************************************************************/
	function __construct ()
	{
		...
		parent::__construct(
			array(
				'singular'  => 'screening',  // singular name of the listed records
				'plural'    => 'screenings', // plural name of the listed records
	...
	**************************************************************************/
	function column_title ($item)
	{
		//Build row actions
		$actions = array(
			'edit'       => sprintf('<a href="?page=%2$s&action=%3$s&screening=%4$s">%1$s</a>',
				/*$1%s*/ __('Edit', 'movdb'),
				/*$2%s*/ $_REQUEST['page'],
				/*$3%s*/ 'edit',
				/*$4%s*/ $item['id']
			),
			'delete'     => sprintf('<a href="?page=%2$s&action=%3$s&screening=%4$s">%1$s</a>',
				/*$1%s*/ __('Delete', 'movdb'),
				/*$2%s*/ $_REQUEST['page'],
				/*$3%s*/ 'delete',
				/*$4%s*/ $item['id']
	...
	/**
	 * Formatiert die Spalte "Datum".
	 */
	function column_date ($item)
	{
		$elements = explode('-', $item['date']);
		$date = mktime(0, 0, 0, $elements[1], $elements[2], $elements[0]);
	...
	/**
	 * Formatiert die Spalte "Gäste".
	 */
	function column_guests ($item)
	{
		return movdb_format_guests($item['guests']);
	}
	...
	function process_bulk_action ()
	{
		switch ($this->current_action())
		{
			case 'show_screenings':
				movdb_redirect(admin_url('admin.php?page=movdb_screenings_page&movies=' . implode(',', $_REQUEST['screening'])));
				exit;
				
			case 'show_sources':
				movdb_redirect(admin_url('admin.php?page=movdb_sources_page&movies=' . implode(',', $_REQUEST['screening'])));
				exit;
		}
	}
	...
	**************************************************************************/
	function prepare_items ()
	{
		...
		*/
		...
		$orderby = (!empty($_REQUEST['orderby'])) ? $_REQUEST['orderby'] : 'date'; // if no sort, set default here
		$order = (!empty($_REQUEST['order'])) ? $_REQUEST['order'] : 'desc'; // if no order, default to desc
		
		// Special order when sorting by date: first by date, then by id (for correct order with more than one movie by day)
		if ($orderby == 'date') {
			$orderby = 'date ' . $order . ', id';
		}
		
		$query  = 'select scr.id, scr.movie, mov.title, mov.org_title, mov.short_title, mov.version, scr.date, scr.guests, ';
		$query .= 'trim(replace(replace(replace(replace(mov.title, "Der", ""), "Die", ""), "Das", ""), "The", "")) as title_sort ';
		$query .= 'from ' . $wpdb->prefix . 'movdb_screenings scr ';
		$query .= 'left outer join ' . $wpdb->prefix . 'movdb_movies mov on mov.id = scr.movie ';
		...
		foreach ($rawdata as $record)
		{
			...
			if (empty($record['guests'])) {
				$this->items_alone++;
			} else {
				$this->items_with_guests++;
			}
			
			// Filterkriterien anwenden und Datensatz ggfl. überspringen
			
			if ($this->get_current_filter() == 'alone') {
				if (!empty($record['guests'])) {
					continue;
				}
			}
			
			if ($this->get_current_filter() == 'with_guests') {
				if (empty($record['guests'])) {
					continue;
				}
			}
	...
	/**
	 * Gibt die Gesamtzahl der Vorführungen in der Datenbank zurück.
	 */
	...
	/**
	 * Gibt die Anzahl der Vorführungen zurück, die alleine gesehen wurden.
	 */
	function get_items_alone ()
	{
		return $this->items_alone;
	}
	
	/**
	 * Gibt die Anzahl der Vorführungen zurück, die mit Gästen gesehen wurden.
	 */
	function get_items_with_guests ()
	{
		return $this->items_with_guests;
	}
	...
	{
		if (!isset($_REQUEST['screening_filter'])) {
			return false;
		}
		
		return $_REQUEST['screening_filter'];
	}
}