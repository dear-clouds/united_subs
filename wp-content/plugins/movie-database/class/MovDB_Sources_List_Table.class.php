 ...
 */
class MovDB_Sources_List_Table extends WP_List_Table
{
	/**
	 ...
	 */
	var $items_total = 0;
	var $items_type_bluray = 0;
	var $items_type_dvd = 0;
	var $items_type_file = 0;
	var $items_video_3d = 0;
	var $items_audience_capable = 0;
	...
	***************************************************************************/
	function __construct ()
	{
		...
		parent::__construct(
			array(
				'singular'  => 'source',  // singular name of the listed records
				'plural'    => 'sources', // plural name of the listed records
	...
	**************************************************************************/
	function column_title ($item)
	{
		//Build row actions
		$actions = array(
			'edit'       => sprintf('<a href="?page=%2$s&action=%3$s&source=%4$s">%1$s</a>',
				/*$1%s*/ __('Edit', 'movdb'),
				/*$2%s*/ $_REQUEST['page'],
				/*$3%s*/ 'edit',
				/*$4%s*/ $item['id']
			),
			'delete'     => sprintf('<a href="?page=%2$s&action=%3$s&source=%4$s">%1$s</a>',
				/*$1%s*/ __('Delete', 'movdb'),
				/*$2%s*/ $_REQUEST['page'],
				/*$3%s*/ 'delete',
				/*$4%s*/ $item['id']
	...
	/**
	 * Formatiert die Spalte "Tonqualität".
	 */
	function column_audio_quality ($item)
	{
		return $this->format_rating_column($item['audio_quality']);
	}
	
	/**
	 * Formatiert die Spalte "Bildqualität".
	 */
	function column_video_quality ($item)
	{
		return $this->format_rating_column($item['video_quality']);
	}
	...
	/**
	 * Formatiert Spalten, die eine Bewertung darstellen.
	 */
	function format_rating_column ($value)
	{
		return movdb_format_rating($value);
	}
	...
	**************************************************************************/
	function get_columns ()
	{
		return array(
			...
			'type'             => __('Type', 'movdb'),
			'audio_quality'    => __('Sound quality', 'movdb'),
			'video_quality'    => __('Image quality', 'movdb'),
	...
	**************************************************************************/
	function get_sortable_columns ()
	{
		return array(
			...
			'audio_quality' => array('audio_quality', false),
			'video_quality' => array('video_quality', false)
		);
	}
	...
	**************************************************************************/
	function prepare_items ()
	{
		...
		*/
		...
		$query  = 'select src.id, src.movie, mov.title, mov.org_title, mov.short_title, mov.version, mov.series, mov.part, mov.year, src.type, src.audio_quality, src.video_quality, src.video_3d, src.audience_capable, ';
		$query .= 'trim(replace(replace(replace(replace(mov.title, "Der", ""), "Die", ""), "Das", ""), "The", "")) as title_sort ';
		$query .= 'from ' . $wpdb->prefix . 'movdb_sources src ';
		$query .= 'left outer join ' . $wpdb->prefix . 'movdb_movies mov on mov.id = src.movie ';
		...
		if (isset($_REQUEST['movies'])) {
			...
			$search = $_REQUEST['s'];
			$query .= 'where mov.title like "%' . $search . '%" or mov.org_title like "%' . $search . '%" or mov.short_title like "%' . $search . '%" ';
		}
		...
		foreach ($rawdata as $record)
		{
			...
			if ($record['type'] == 'blu-ray') {
				$this->items_type_bluray++;
			}
			...
			if ($record['type'] == 'file') {
				$this->items_type_file++;
			}
			
			if ($record['video_3d'] == 'true') {
				$this->items_video_3d++;
			}
			
			if ($record['audience_capable'] == 'true') {
				$this->items_audience_capable++;
			}
			
			// Filterkriterien anwenden und Datensatz ggfl. überspringen
			
			if ($this->get_current_filter() == 'type_bluray') {
				if ($record['type'] != 'blu-ray') {
					continue;
				}
			}
			
			if ($this->get_current_filter() == 'type_dvd') {
				if ($record['type'] != 'dvd') {
					continue;
				}
			}
			
			if ($this->get_current_filter() == 'type_file') {
				if ($record['type'] != 'file') {
					continue;
				}
			}
			
			if ($this->get_current_filter() == 'video_3d') {
				if ($record['video_3d'] != 'true') {
					continue;
				}
			}
			
			if ($this->get_current_filter() == 'audience_capable') {
				if ($record['audience_capable'] != 'true') {
					continue;
				}
			}
	...
	/**
	 * Gibt die Anzahl der Quellen vom Typ Blu-ray zurück.
	 */
	function get_items_type_bluray ()
	{
		return $this->items_type_bluray;
	}
	
	/**
	 * Gibt die Anzahl der Quellen vom Typ DVD zurück.
	 */
	function get_items_type_dvd ()
	{
		return $this->items_type_dvd;
	}
	
	/**
	 * Gibt die Anzahl der Quellen vom Typ Datei zurück.
	 */
	function get_items_type_file ()
	{
		return $this->items_type_file;
	}
	
	/**
	 * Gibt die Anzahl der 3D-Quellen zurück.
	 */
	function get_items_video_3d ()
	{
		return $this->items_video_3d;
	}
	
	/**
	 * Gibt die Anzahl der Quellen zurück, die geeignet sind, um sie einem Publikum vorzuführen.
	 */
	function get_items_audience_capable ()
	{
		return $this->items_audience_capable;
	}
	...
	{
		if (!isset($_REQUEST['source_filter'])) {
			return false;
		}
		
		return $_REQUEST['source_filter'];
	}
}