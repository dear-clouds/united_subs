 ...
 */
class MovDB_SuggestionsBest_List_Table extends WP_List_Table
{
	/**
	...
	*/
	function extra_tablenav ($which)
	{
		...
		echo '<label for="rating_threshold">' . __('Rating threshold', 'movdb') . '</label>';
		echo '<input type="range" min="1" max="10" step="1" value="' . $this->get_current_rating_threshold() . '" name="rating_threshold" id="rating_threshold" />';
	...
	**************************************************************************/
	function prepare_items ()
	{
		...
		*/
		...
		$query .= 'where mov.id in (select distinct movie from ' . $wpdb->prefix . 'movdb_screenings) ';
		$query .= 'and rating >= ' . $this->get_current_rating_threshold() . ' ';
	...
	function get_current_rating_threshold ()
	{
		return isset($_GET['rating_threshold']) ? intval($_GET['rating_threshold']) : 8;
	}
}