...
if ( class_exists('BP_Group_Extension' ) ) {
	...
	class Group_External_Blogs extends BP_Group_Extension {
		...
		function __construct() {
			...
			$this->name = __( 'Sorties & News', 'bp-groups-externalblogs' );
	...
	function bp_groupblogs_fetch_group_feeds( $group_id = false ) {
		...
		foreach ( (array) $items as $post_date => $post ) {
			...
			$activity_action = sprintf( __( '%s @ %s de %s', 'bp-groups-externalblogs' ), '<a class="feed-link" href="' . esc_attr( $post['link'] ) . '">' . esc_attr( $post['title'] ) . '</a>', '<a class="feed-author" href="' . esc_attr( $post['blogurl'] ) . '">' . attribute_escape( $post['blogname'] ) . '</a>', '<a href="' . bp_get_group_permalink( $group ) . '">' . attribute_escape( $group->name ) . '</a>' );
