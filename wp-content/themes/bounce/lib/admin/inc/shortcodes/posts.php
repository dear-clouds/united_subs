		...
		<div class="post-wrapper <?php echo $name.' '.$spacing; ?>">
			...
			?>
				...
				<div <?php post_class('post-loop'.$preload.$first_column.$columns); ?> style="width: <?php echo $col_width; ?>%;">
						...
						<div class="post-thumbnail<?php if($image_wrap == "true") { ?> wrap<?php } ?>">
							...
							<?php } ?>
								...
								<?php if($link == "image" OR $link == "both") { ?>
									<?php if(get_post_meta(get_the_ID(), 'ghostpool_link_type', true) == "Lightbox Image") { ?><span class="hover-image"></span><?php } elseif(get_post_meta(get_the_ID(), 'ghostpool_link_type', true) == "Lightbox Video") { ?><span class="hover-video"></span><?php } ?>
								<?php } ?>
							...
							<?php if(($link == "image" OR $link == "both") && get_post_meta(get_the_ID(), 'ghostpool_link_type', true) != "None") { ?></a><?php } ?>
					...
					<?php if(get_post_meta(get_the_ID(), 'ghostpool_link_type', true) == "Lightbox Image" OR get_post_meta(get_the_ID(), 'ghostpool_link_type', true) == "Lightbox Video") { ?>
					...
					<div class="post-text">
						...
						<?php if($title == "true") { ?><h2 style="<?php echo $title_size.$title_font.$title_weight; ?>"><?php if($link == "title" OR $link == "both") { ?><a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"><?php } ?><?php the_title(); ?><?php if($link == "title" OR $link == "both") { ?></a><?php } ?></h2><?php } ?>
