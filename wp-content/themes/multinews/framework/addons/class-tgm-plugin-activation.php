 ...
 * @version   2.5.2 - custom for Momizat themes
    ...
    class TGM_Plugin_Activation {
        ...
        public function init() {
            ...
            if ( $this->plugins ) {
                ...
                add_action( 'admin_notices', array( $this, 'custom_notices' ) );
                //add_action( 'admin_menu', array( $this, 'update_count' ) );
                if ( $this->has_notices ) {
//                    add_action( 'admin_notices', array( $this, 'notices' ) );
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' == $_GET['tgmpa-install'] ) ) {
                ...
                if ( $this->is_automatic ) {
                    ...
                    if ( is_wp_error( $activate ) ) {
                        ...
                        if (isset($_GET['plugin_source']) && $_GET['plugin_source'] == 'repo') {
                            echo '<p><a href="' . add_query_arg( 'page', 'recommended_plugins', admin_url( 'admin.php' ) ) . '" title="' . esc_attr( __('Return to Plugins', 'theme') ) . '" target="_parent">' . __('Return to Plugins', 'theme') . '</a></p>';
                        } else {
                            echo '<p><a href="' . add_query_arg( 'page', 'momizat-panel', admin_url( 'admin.php' ) ) . '" title="' . esc_attr( __('Return to Addons', 'theme') ) . '" target="_parent">' . __('Return to Addons', 'theme') . '</a></p>';
                        }
        ...
        */
            ...
            if ( ! empty( $message ) ) {
                ...
                if (empty($show_install_link) && $update_link) {
                  //$show_install_link  = '<a href="' . esc_url(add_query_arg( 'page', $this->menu, admin_url( 'themes.php' ) )) . '">' . translate_nooped_plural( $this->strings['update_link'], $update_link_count, 'tgmpa' ) . '</a>';
                  $show_install_link  = '<a href="' . esc_url(add_query_arg( 'page', $this->page_slug, admin_url( 'admin.php' ) )) . '">' . translate_nooped_plural( $this->strings['update_link'], $update_link_count, 'tgmpa' ) . '</a>';
    ...
    foreach ($menu as $key => $value) {
        if ($menu[$key][2] == 'momizat-panel') {
            $menu[$key][0] .= " <span class='update-plugins count-$update_count'><span class='plugin-count'>" . $update_count . '</span></span>';
            return;
        }
    }
       }
        ...
        /**
         * Determine if we're on the Momizat addon page.
         *
         * @since 2.1.0
         *
         * @return boolean True when on the TGMPA page, false otherwise.
